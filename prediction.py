...
def prediction():
    ...
    database_url = 'mysql://root@localhost:3306/nurtura_grow'
    ...
    columns_to_select = ['timestamp_pengukuran', 'kelembapan_tanah', 'kelembapan_udara', 'suhu']
    ...
    df = df.rename(columns={'timestamp_pengukuran': 'Time', 'kelembapan_tanah': 'SoilMoisture', 'kelembapan_udara': 'Humidity', 'suhu': 'temperature'})
    ...
    train_soil_moisture, test_soil_moisture = train_test_split(df_soil_moisture, test_size=0.2)
    train_humidity, test_humidity = train_test_split(df_humidity, test_size=0.2)
    train_temperature, test_temperature = train_test_split(df_temperature, test_size=0.2)
    ...
    predicted_soil_moisture_1_hour_arima = arima_predict(df_soil_moisture, 1, order_soil_moisture)
    ...
    predicted_temperature_1_hour_arima = arima_predict(df_temperature, 1, order_temperature)
    ...
    predicted_json = predicted_data.to_json(orient='records', date_format='iso', default_handler=str)

    # Format time in the JSON
    json_dict = json.loads(predicted_json)
    json_dict[0]['Time'] = pd.to_datetime(json_dict[0]['Time']).strftime('%Y-%m-%d %H:%M:%S')

    # Convert back to JSON with the desired time format
    formatted_json = json.dumps(json_dict, default=str)
